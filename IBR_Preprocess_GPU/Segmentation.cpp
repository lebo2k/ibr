...
void Segmentation::InitToCidOut(int cid_out) {
	cid_out_ = cid_out;

	num_pixels_out_ = sd_->num_pixels_[cid_out_];
	num_unknown_pixels_out_ = sd_->num_unknown_pixels_[cid_out_];

	int h_adj = sd_->heights_[cid_out_] - 2 * GLOBAL_PLNSEG_WINDOW;
	int w_adj = sd_->widths_[cid_out_] - 2 * GLOBAL_PLNSEG_WINDOW;
	num_pixels_out_adj_ = h_adj * w_adj;

	unknown_mask_out_ = sd_->masks_unknowns_[cid_out_];
	unknown_mask_out_.resize(sd_->heights_[cid_out_], sd_->widths_[cid_out_]);
	...
	for (int c = 0; c < mask_unknown_tmp.cols(); c++) {
		for (int r = 0; r < mask_unknown_tmp.rows(); r++) {
			if ((c < GLOBAL_PLNSEG_WINDOW) ||
				(c >= sd_->widths_[cid_out_] - GLOBAL_PLNSEG_WINDOW) ||
				(r < GLOBAL_PLNSEG_WINDOW) ||
				(r >= sd_->heights_[cid_out_] - GLOBAL_PLNSEG_WINDOW)) {
				if (*pM) {
					*pM = false;
...
void Segmentation::ComputeGBSegmentation(Matrix<unsigned int, Dynamic, 1> *seg_labels, Mat *img, int sigma, int k, int min_size, int height, int width, bool compress) {
	...
	assert(seg_labels->rows() == num_pixels);
...
void Segmentation::ComputeMeanShiftSegmentation(Matrix<unsigned int, Dynamic, 1> *seg_labels, Mat *img, int sigmaS, float sigmaR, int minRegion, int height, int width) {
	...
	assert(seg_labels->rows() == width*height);
...
// updates seg_labels with the result
// seg_labels must have width*height rows
void Segmentation::ComputeMeanShiftSegmentation_Grayscale(Matrix<unsigned int, Dynamic, 1> *seg_labels, Mat *img, int sigmaS, float sigmaR, int minRegion, int height, int width) {
	bool debug = true;
	...
	// Read in the input image
	uint8_t *temp_im = new uint8_t[height * width];
	uint8_t *B = temp_im;
	...
	im_proc.DefineImage(temp_im, GRAYSCALE, height, width);
	delete[] temp_im;
	if (im_proc.ErrorStatus == EL_ERROR)
		cerr << im_proc.ErrorMessage << endl;
...
void Segmentation::SegmentPlanar_ePhoto(const float f, Eigen::Matrix<float, Dynamic, 3> *scratch, Eigen::Matrix<float, Dynamic, 1> *X) {
	assert(X->rows() == num_unknown_pixels_out_);
...
void Segmentation::SegmentPlanar_ePhoto(const Eigen::Matrix<float, Dynamic, 3> *F, Eigen::Matrix<float, Dynamic, 3> *scratch, Eigen::Matrix<float, Dynamic, 1> *X) {
	...
	assert(F->rows() == X->rows() && F->rows() == num_unknown_pixels_out_);
	
	double col_thresh = GLOBAL_LABELING_ENERGY_COL_THRESHOLD;
	double factor = -1. / (col_thresh*3.);
	double log2 = log(2);

	(*scratch) = (*F) - sd_->Aunknowns_[cid_out_];
...
void Segmentation::SegmentPlanar_AvgFilter(Eigen::Matrix<float, Dynamic, Dynamic> *Y, Eigen::Matrix<float, Dynamic, Dynamic> *scratch, Eigen::Matrix<float, Dynamic, 1> *Yavg, int height, int width, int window) {
	...
	assert(Y->rows() == height && Y->cols() == width);
	assert(scratch->rows() == height && scratch->cols() == width);
	assert(Yavg->rows() == num_pixels_out_adj_);
...
// assumes any non-unknown pixel neighbors' photoconsistency energy values are constant zero because we assume their depths are perfectly known
...
void Segmentation::SegmentPlanar_AvgFilter_Unknowns(Eigen::Matrix<float, Dynamic, 1> *Y, Eigen::Matrix<float, Dynamic, Dynamic> *scratch1, Eigen::Matrix<float, Dynamic, Dynamic> *scratch2, Eigen::Matrix<float, Dynamic, Dynamic> *scratch3, Eigen::Matrix<float, Dynamic, 1> *Yavg, int window) {
	...
	int height = sd_->heights_[cid_out_];
	int width = sd_->widths_[cid_out_];
	int hadj = sd_->heights_[cid_out_] - 2 * window;
	int wadj = sd_->widths_[cid_out_] - 2 * window;
	assert(Y->rows() == num_unknown_pixels_out_);
	assert(scratch1->rows() == height && scratch1->cols() == width);
	assert(scratch2->rows() == hadj && scratch2->cols() == wadj);
	assert(scratch3->rows() == height && scratch3->cols() == width);
	assert(Yavg->rows() == num_unknown_pixels_out_adj_);
	...
	Mat Ycv = cv::Mat::zeros(height, width, CV_32F);
	...
	pS2 += window * height;
	unsigned int *pI = sd_->Iunknowns_[cid_out_].data(); // for pixels with unknown depths, column-major indices into full image for compact unknown pixel representation (compact number of them of full-image pixel indices)
	unsigned int idx_full;
	Point pt;
	for (int i = 0; i < num_unknown_pixels_out_; i++) {
		idx_full = *pI++;
		pt = PixIndexBwdCM(idx_full, sd_->heights_[cid_out_]);
		if ((pt.y < window) ||
			(pt.y >= width - window))
	...
	// apply along columns to result from applying along rows
	(*scratch2) *= mult;
	pI = sd_->Iunknowns_[cid_out_].data();
	for (int i = 0; i < num_unknown_pixels_out_; i++) {
		idx_full = *pI++;
		pt = PixIndexBwdCM(idx_full, sd_->heights_[cid_out_]);
		if ((pt.y < window) ||
...
void Segmentation::SegmentPlanar_CwiseMin(const Eigen::Matrix<float, Dynamic, 1> *X, int index_X, Eigen::Matrix<float, Dynamic, 1> *min, Eigen::Matrix<int, Dynamic, 1> *min_indices) {
	assert(X->rows() == min->rows() && X->rows() == min_indices->rows());
...
void Segmentation::SegmentPlanar_CorrDisparities(Matrix<float, Dynamic, 1> *corr) {
	assert(corr->rows() == num_unknown_pixels_out_);
	...
	if (debug) cout << "Segmentation::SegmentPlanar_CorrDisparities()" << endl;
	...
	WC.col(0) = sd_->Xunknowns_[cid_out_].cast<float>();
	WC.col(1) = sd_->Yunknowns_[cid_out_].cast<float>();
	WC.col(2).setOnes();
	...
	Matrix<float, Dynamic, Dynamic> scratch1_SegmentPlanar_AvgFilter(sd_->heights_[cid_out_], sd_->widths_[cid_out_]);
	Matrix<float, Dynamic, Dynamic> scratch2_SegmentPlanar_AvgFilter(sd_->heights_[cid_out_] - 2 * GLOBAL_PLNSEG_WINDOW, sd_->widths_[cid_out_] - 2 * GLOBAL_PLNSEG_WINDOW);
	Matrix<float, Dynamic, Dynamic> scratch3_SegmentPlanar_AvgFilter(sd_->heights_[cid_out_], sd_->widths_[cid_out_]);
	Matrix<float, Dynamic, 1> Yavg(num_unknown_pixels_out_adj_, 1);
	Matrix<float, Dynamic, 1> Ysum(num_unknown_pixels_out_adj_, 1);

	Matrix<bool, Dynamic, 3> InboundsMask(num_unknown_pixels_out_, 3); // used to determine whether reprojection is masked in or out of destination screen space for cameras not otherwise included in stereo reconstruction
	...
	for (std::map<int, Mat>::iterator it = sd_->imgsT_.begin(); it != sd_->imgsT_.end(); ++it) {
		int cid = (*it).first;
		//if (std::find(sd_->use_cids_[cid_out_].begin(), sd_->use_cids_[cid_out_].end(), cid) == sd_->use_cids_[cid_out_].end()) continue; // will need non-used cids when prepping to call InterpolateAgainstMask in the next loop, so don't leave in this restriction for used cids
		if (cid == cid_out_) continue; // notes in ojw_segpln.m say the images list used here excludes the reference image, but his code doesn't actually exclude it

		// project the points
		Eigen::Matrix<float, Dynamic, 3> X(num_unknown_pixels_out_, 3);
		X = WC * sd_->Pout2ins_[cid_out_][cid].block<3, 3>(0, 0).transpose(); //  take homogeneous screen space coordinates and project them, sort of; num_pixelsx3 result
		Xs[cid] = X;
		Eigen::Matrix<float, 3, 1> P4;
		P4 = sd_->Pout2ins_[cid_out_][cid].block<3, 1>(0, 3); // save last column of P separately
		P4s[cid] = P4;
	}

	if (debug) cout << "completed set up camera-specific matrices for use in the following loop" << endl;
	
	if (timing_loop_disp) t_loop_disp = (double)getTickCount();
	for (int b = 0; b < sd_->disps_[cid_out_].size(); b++) {
		Ysum.setZero();
		InboundsMask = sd_->unknown_disps_valid_[cid_out_].col(b).replicate(1, 3);

		for (std::map<int, Mat>::iterator it = sd_->imgsT_.begin(); it != sd_->imgsT_.end(); ++it) {

			int cid = (*it).first;
			if (std::find(sd_->use_cids_[cid_out_].begin(), sd_->use_cids_[cid_out_].end(), cid) == sd_->use_cids_[cid_out_].end()) continue;
			if (cid == cid_out_) continue; // notes in ojw_segpln.m say the images list used here excludes the reference image, but his code doesn't actually exclude it

			// Vary image coordinates according to disparity
			d = sd_->disps_[cid_out_](b) * P4s[cid];
			...
			Interpolation::Interpolate<float>(sd_->imgsT_[cid].cols, sd_->imgsT_[cid].rows, &sd_->As_[cid], &oX, &oY, &sd_->masks_[cid], -1000, &Ainterp); // interpolate reprojected floating point image coordinates in input image cid (reprojected from reference image) to interpolate cid input image colors of surrounding pixels; use masks not masks_unknowns_ here
			Ainterp = (InboundsMask.array() == false).select(Matrix<float, Dynamic, 3>::Constant(Ainterp.rows(), 3, -1000), Ainterp); // update Ainterp to have oobv where InboundsMask is false to accomodate masks of screen spaces of cameras not otherwise included in the planar segmentation
		...
		if ((timing_loop_disp) &&
			...
			cout << "Segmentation::SegmentPlanar_Corr() loop time for set of " << timing_set_disp_loops << " loops ending at disparity " << b << " = " << t_loop_disp*1000. / getTickFrequency() << " ms (of " << sd_->disps_[cid_out_].size() << " disparities)" << endl;
			t_loop_disp = (double)getTickCount(); // reset timer for next batch
		}
	}

	if (debug) DebugPrintMatrix(&corr_min, "corr_min");
	
	// Normalize and extract highest scoring matches(winner takes all)
	SegmentPlanar_ePhoto(-1000., &scratch_SegmentPlanar_ePhoto, &Y);
	int num_input_images = sd_->use_cids_[cid_out_].size();
	...
	for (int r = 0; r < num_unknown_pixels_out_adj_; r++) {
		...
		corr_adj(r, 0) = (float)sd_->disps_[cid_out_](idx_disp, 0);
	}
	...
	corr_full.resize(sd_->heights_[cid_out_], sd_->widths_[cid_out_]);
	corr_full.row(0) = corr_full.row(3);
	corr_full.row(1) = corr_full.row(2);
	corr_full.row(sd_->heights_[cid_out_] - 1) = corr_full.row(sd_->heights_[cid_out_] - 4);
	corr_full.row(sd_->heights_[cid_out_] - 2) = corr_full.row(sd_->heights_[cid_out_] - 3);
	corr_full.col(0) = corr_full.col(3);
	corr_full.col(1) = corr_full.col(2);
	corr_full.col(sd_->widths_[cid_out_] - 1) = corr_full.col(sd_->widths_[cid_out_] - 4);
	corr_full.col(sd_->widths_[cid_out_] - 2) = corr_full.col(sd_->widths_[cid_out_] - 3);
	corr_full.resize(num_pixels_out_, 1);

	if (debug) {
		DebugPrintMatrix(&corr_full, "corr_full");
		DisplayImages::DisplayGrayscaleImage(&corr_full, sd_->heights_[cid_out_], sd_->widths_[cid_out_]);
	}

	(*corr) = sd_->ContractFullToUnknownSize(cid_out_, &corr_full);
...
void Segmentation::SegmentPlanar_GenSegs() {
	...
	Matrix<unsigned int, Dynamic, Dynamic> segs = sd_->segs_[cid_out_];
	segs.resize(segs.rows()*segs.cols(), 1);
	seg_labels_.resize(num_unknown_pixels_out_, 1);
	seg_labels_ = sd_->ContractFullToUnknownSize(cid_out_, &segs);
	DetermineUniqueUnknownSegLabels();
	...
	Matrix<unsigned int, Dynamic, Dynamic> seg_labels_full(num_pixels_out_, nMaps);
	Mat imgT_out_masked = cv::Mat::zeros(sd_->imgsT_[cid_out_].rows, sd_->imgsT_[cid_out_].cols, CV_8UC3);
	Matrix<bool, Dynamic, Dynamic> mask = sd_->masks_[cid_out_];
	mask.resize(sd_->imgsT_[cid_out_].rows, sd_->imgsT_[cid_out_].cols);
	StereoData::MaskImg(&sd_->imgsT_[cid_out_], &mask, &imgT_out_masked);
	mask.resize(0, 0);
	for (int b = 0; b < nMaps; b++) {
		...
		if (b < 8) {// segment the image using mean shift
			...
			ComputeMeanShiftSegmentation(&sl_tmp, &imgT_out_masked, sp(0, 0), sp(1, 0), sp(2, 0), sd_->heights_[cid_out_], sd_->widths_[cid_out_]);
		}
			...
			ComputeGBSegmentation(&sl_tmp, &imgT_out_masked, 0, sp(3, 0), sp(2, 0), sd_->heights_[cid_out_], sd_->widths_[cid_out_], 1);
		}
		seg_labels_full.block(0, b, seg_labels_full.rows(), 1) = sl_tmp;
	}
	sl_tmp.resize(0, 1);
	
	seg_labels_ = sd_->ContractFullToUnknownSize(cid_out_, &seg_labels_full);
...
void Segmentation::SegmentPlanar_WCPlaneFitting(Matrix<float, Dynamic, 1> *corr_disparities, Matrix<float, Dynamic, 3> *WC) {
	...
	assert(WC->rows() == num_unknown_pixels_out_);
	assert(corr_disparities->rows() == num_unknown_pixels_out_ && corr_disparities->cols() == 1);
	...
	WC->block(0, 1, num_unknown_pixels_out_, 1) = WC->block(0, 2, num_unknown_pixels_out_, 1).cwiseProduct(sd_->Yunknowns_[cid_out_].cast<float>());
	WC->block(0, 0, num_unknown_pixels_out_, 1) = WC->block(0, 2, num_unknown_pixels_out_, 1).cwiseProduct(sd_->Xunknowns_[cid_out_].cast<float>());
...
// determines labels that include at least one unknown pixel
// updates unknown_seg_labels_, which is a map of map size => vector of segmentation labels from seg_labels_ with column==map size and which apply to at least one unknown pixel; labels are sorted and unique
// requires that seg_labels_ has already been initialized
void Segmentation::DetermineUniqueUnknownSegLabels() {
	assert(seg_labels_.rows() > 0 && seg_labels_.cols() > 0);
	
	unsigned int label;
	for (int b = 0; b < seg_labels_.cols(); b++) { // go through each map size
		std::vector<unsigned int> labels;
		for (int r = 0; r < num_unknown_pixels_out_; r++) { // go through each unknown pixel
			label = seg_labels_(r, b);
			labels.push_back(label);
		}

		// efficient sort/unique method if number of duplicates is small
		//std::sort(labels.begin(), labels.end());
		//labels.erase(unique(labels.begin(), labels.end()), labels.end());

		// efficient sort/unique method if number of duplicates is large (manual conversion to a sorted, unique set is faster than using the built-in constructor)
		set<int> s;
		unsigned size = labels.size();
		for (unsigned i = 0; i < size; ++i) s.insert(labels[i]);
		labels.assign(s.begin(), s.end());

		unknown_seg_labels_[b] = labels;
...
void Segmentation::SegmentPlanar_GenMaps(Eigen::Matrix<float, Dynamic, 3> *WC) {
	...
	assert(WC->rows() == num_unknown_pixels_out_ && WC->cols() == 3);

	sd_->D_segpln_.resize(0, 0);
	sd_->D_segpln_ = Matrix<float, Dynamic, Dynamic>(num_unknown_pixels_out_, seg_labels_.cols());
	...
	float no_ls_soln_val = 1e-100; // QR decomposition to find the least squares best-fit fronto-parallel plane may fail and return nan values...in that case, we set the planar disparity values to a small constant, given here

	for (std::map<int, std::vector<unsigned int>>::iterator itb = unknown_seg_labels_.begin(); itb != unknown_seg_labels_.end(); ++itb) { // for each map size
		int b = (*itb).first;
		if (timing_mapsize_loop) t_mapsize_loop = (double)getTickCount();
		int num_labels = (*itb).second.size();


		if (debug) {
			cout << "Map size " << b << " has " << num_labels << " unique labels that are applied to at least one unknown pixel" << endl;
			Matrix<unsigned int, Dynamic, 1> seg_labels_b = seg_labels_.block(0, b, seg_labels_.rows(), 1);
			Matrix<unsigned int, Dynamic, 1> seg_labels_full = sd_->ExpandUnknownToFullSize(cid_out_, &seg_labels_b);
			DisplayImages::DisplaySegmentedImage(&seg_labels_full, sd_->heights_[cid_out_], sd_->widths_[cid_out_]);
		}
		
		int label_counter = 0; // since 'a' below is the label number, which skips around for unknown pixels, use a counter to keep track of how many labels we've mapped
		if (timing_label_loop) t_label_loop = (double)getTickCount();
		for (std::vector<unsigned int>::iterator itl = (*itb).second.begin(); itl != (*itb).second.end(); ++itl) { // for each unknown pixel label
			label_counter++;
			unsigned int a = (*itl);
			bool no_ls_soln = false;
			...
			if (debug_loop) {
				...
				cout << "M count before excluding homogeneous 0 values " << points_used << endl;
			}

			if (debug_loop) { // display the points with the current label
				int val;
			...
			// N = N(N(:,3)~=0,:);
			bool *pM = M.data();
			bool *pWCnzh = WC_nonzero_homogeneous.data();
			points_used = 0;
			for (int r = 0; r < num_unknown_pixels_out_; r++) {
				if ((*pM) &&
					(*pWCnzh)) {
					N.row(points_used) = WC->row(r);
					points_used++;
				}
				pM++;
				pWCnzh++;
			...
			if (debug_loop) {
				cout << "M count after excluding homogeneous 0 values " << points_used << endl;
				cout << endl << "N.block(0, 0, points_used, 3)" << endl << N.block(0, 0, points_used, 3) << endl; // display the points with the current label
				cin.ignore();
			}
			...
			if (points_used > 3) { // if more than 3 points are in the list, weed out outliers using RANSAC before determining best-fit plane from remaining inliers
				// M_ = rplane(N, rt);
				RPlane(&N, &scratch_dist_RPlane, &scratch_v_RPlane, &scratch_rm_RPlane, &scratch_pv_RPlane, rt, num_unknown_pixels_out_, points_used, &M2); // ransac to weed out outliers; returns inliers - coefficients of returned matrix are integer boolean invalues denoted whether a given index into arg pts denotes an inlier; M2 becomes a matrix of boolean coefficients denoting whether the points_used points in N are inliers or outliers...so only the first points_used coefficients in M2 should be used, and they should be used in reference only to the used points ("used" in the sense of constructing this plane, not related to used versus unknown)

				if (debug_loop) {
					cout << endl << "M2.block(0, 0, points_used, 1)" << endl << M2.block(0, 0, points_used, 1) << endl; // display inlier/outlier booleans for points used
					cin.ignore();
				}

				// N = N(M_,:);
				bool *pM2 = M2.data();
				points_inliers = 0; // reduce points of interest in N to include only points for which the corresponding coefficient of M_ is true
				for (int r = 0; r < points_used; r++) {
					if (*pM2++) { // M2 is a matrix of boolean coefficients denoting whether the points_used points in N are inliers or outliers...so only the first points_used coefficients in M2 should be used, and they should be used in reference only to the used points ("used" in the sense of constructing this plane, not related to used versus unknown)
						N.row(points_inliers) = N.row(r); // should work because r>=points_inliers and both start at 0 and are increasing with the loop, though potentially at different rates
						points_inliers++;
					}
				}

				if (debug_loop) {
					cout << "RANSAC found " << points_inliers << "points_inliers of " << points_used << " points_used" << endl;
					cout << "double-check...points_inliers should be " << M2.block(0,0,points_used,1).count() << endl;
				}
			}
			else if ((points_used == 0) ||
					(points_inliers == 0)) { // to prevent errors, though shouldn't occur; actually, will occur when using masking
				//cerr << "Segmentation::SegmentPlanar_GenMaps() points_used == 0 || points_inliers == 0 for map size " << b << " and label " << a << ".  No work done on proposal this iteration." << endl;
				continue;
			}

			// find least squares plane from inliers
			if (debug_loop) {
				cout << endl<< "N.block(0, 0, points_inliers, 3)" << endl << N.block(0, 0, points_inliers, 3) << endl;
				cin.ignore();
			}
			rm.setConstant(-1.);
			...
			int j = 0;
			double *pX = sd_->Xunknowns_[cid_out_].data();
			double *pY = sd_->Yunknowns_[cid_out_].data();
			for (int r = 0; r < num_unknown_pixels_out_; r++) {
				if (M(r, 0)) {
					X(j, 0) = (float)*pX;
					Y(j, 0) = (float)*pY;
			...
			if (no_ls_soln) D_segpln_unk.setConstant(no_ls_soln_val);
			else {
			...
			sd_->D_segpln_.block(0, b, num_unknown_pixels_out_, 1) += D_segpln_unk;
			
			if (debug_loop) {
				Eigen::Matrix<float, Dynamic, 1> dispm = sd_->D_segpln_.block(0, b, sd_->D_segpln_.rows(), 1);
				Eigen::Matrix<float, Dynamic, 1> dispm_full = sd_->ExpandUnknownToFullSize(cid_out_, &dispm);
				DisplayImages::DisplayGrayscaleImage(&dispm_full, sd_->heights_[cid_out_], sd_->widths_[cid_out_]); // can have values outside of range of expected disparities (including negative values), so don't display as a disparity image?
			}

			if ((timing_label_loop) &&
				(label_counter%timing_set_label_loops == 0)) {
				t_label_loop = (double)getTickCount() - t_label_loop;
				cout << "Segmentation::SegmentPlanar_GenMaps() label loop running time for mapsize index " << b << " for set of " << timing_set_label_loops << " loops ending at label index " << label_counter << " = " << t_label_loop*1000. / getTickFrequency() << " ms (of " << num_labels << " labels)" << endl;
			}
		}
		...
		if (timing_mapsize_loop) {
			...
			cout << "Segmentation::SegmentPlanar_GenMaps() mapsize loop running time for mapsize index " << b << " = " << t_mapsize_loop*1000. / getTickFrequency() << " ms (of " << unknown_seg_labels_.size() << "map sizes)" << endl;
		}
	}	
...
// updates matrix sd_->D_segpln_ and sets it to size sd_->num_pixels_[cid_out_] x nMaps where nMaps == 14 hardcoded
void Segmentation::SegmentPlanar(int cid_out) {
	bool debug = false;

	bool timing = true; double t;
	if (timing) t = (double)getTickCount();

	if (debug) cout << "Segmentation::SegmentPlanar()" << endl;

	InitToCidOut(cid_out);
	...
	// note that ojw resize D to a 3D matrix of sd_->heights_[cid_out_] x sd_->widths_[cid_out_] x nMaps; skipping that here since Eigen can only represent 2D matrices
	...
	if (debug) {
		for (int b = 0; b < sd_->D_segpln_.cols(); b++) {
			Eigen::Matrix<float, Dynamic, 1> dispm = sd_->D_segpln_.block(0, b, sd_->D_segpln_.rows(), 1);
			Eigen::Matrix<float, Dynamic, 1> dispm_full = sd_->ExpandUnknownToFullSize(cid_out_, &dispm);
			DisplayImages::DisplayGrayscaleImage(&dispm_full, sd_->heights_[cid_out_], sd_->widths_[cid_out_]);
		}
	}
...
void Segmentation::RPlane(const Eigen::Matrix<float, Dynamic, 3> *pts, Eigen::Matrix<float, Dynamic, 1> *scratch_dist, Eigen::Matrix<bool, Dynamic, 1> *scratch_v, Eigen::Matrix<float, Dynamic, 1> *scratch_rm, Eigen::Matrix<float, Dynamic, 3> *scratch_pv, float th, int points_passed, int points_used, Eigen::Matrix<bool, Dynamic, 1> *inls) {
	...
	assert(pts->rows() == points_passed);
	assert(scratch_dist->rows() == points_passed);
	assert(scratch_v->rows() == points_passed);
	assert(scratch_rm->rows() == points_passed);
	assert(scratch_pv->rows() == points_passed);

	bool debug = false;

	double max_sam = 500.; // max sample count
	float conf = 0.95; // confidence
	int max_i = 3;
	int no_i; // number of inliers
	double no_sam = 0.; // number of samples
	...
	while (no_sam < max_sam) {
		...
		*/
		// compute the distance of all points to a best-fit plane with normal N
		...
		no_i = (int)scratch_v->block(0, 0, points_used, 1).count(); // inliers are those within distance threshold th of best-fit plane
		/*
		if (debug) {
		...
		*/
		if (max_i < no_i) { // re-estimate plane and inliers
			...
			// create matrix of points from pts where corresponding v value is true (the inliers, where distance to plane with within threshold th)
			int i = 0;
			for (int r = 0; r < points_used; r++) {
				if ((*scratch_v)(r, 0)) {
			...
			assert(i == no_i); // all inliers should be included here
...
void Segmentation::MeshGrid(int height, int width, Eigen::Matrix<float, Dynamic, Dynamic> &X, Eigen::Matrix<float, Dynamic, Dynamic> &Y) {
	assert(X.rows() == height && X.cols() == width);
	assert(Y.rows() == height && Y.cols() == width);

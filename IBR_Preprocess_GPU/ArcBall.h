...
/**   06/18/2014 - (MAL) - Tweaked for Adornably, Inc.      **/
/**                                                         **/
/*************************************************************/
...
// conversion functions (MAL)
inline static Mat Arc4fTToCV(Matrix4fT *TransIn) {
	Mat TransOut = cv::Mat(Size(4, 4), CV_64F);

	double* p;
	int idx;
	for (int r = 0; r<4; r++) {
		p = TransOut.ptr<double>(r);
		for (int c = 0; c<4; c++) {
			idx = r * 4 + c;
			p[c] = TransIn->M[idx];
		}
...
inline static Matrix4fT CVToArc4fT(Mat *TransIn) {
	Matrix4fT TransOut;

	double* p;
	int idx;
	for (int r = 0; r<4; r++) {
		p = TransIn->ptr<double>(r);
		for (int c = 0; c<4; c++) {
			idx = r * 4 + c;
			TransOut.M[idx] = p[c];
		}
...
//Math functions
	...
	inline
	static void Matrix3fSetRotationFromMatrix4f(Matrix3fT* NewObj, const Matrix4fT* m1)
	{
			assert(NewObj && m1);
